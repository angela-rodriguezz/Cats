...
def choose(paragraphs, select, k):
    ...
    for word in paragraphs:
        if select(word) == True and counter == k:
            return word
        elif select(word) == True:
            counter += 1
    return ''
    # END PROBLEM 1
...
def accuracy(typed, reference):
    ...
    for index in range(min(len(typed_words), len(reference_words))):
        if reference_words[index] == typed_words[index]:
            accuracy_check += 1
    return (accuracy_check / len(typed_words)) * 100
    # END PROBLEM 3
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    for index in range(len(word_list)):
        if word_list[index] == typed_word:
            checker = True
        if diff_function(typed_word, word_list[index], limit) < diff_function(typed_word, word_list[index_returner], limit):
            index_returner = index
    if checker == True or (diff_function(typed_word, word_list[index_returner], limit) > limit):
        return typed_word
    return word_list[index_returner]
    # END PROBLEM 5
...
def sphinx_swaps(start, goal, limit):
    ...
    """
    # BEGIN PROBLEM 6
    def counter(num_counter, index):
        if num_counter > limit:
            return limit + 1
        if index == 0:
            if start[index] != goal[index]:
                return (num_counter + 1) + abs(len(goal) - len(start))
            return num_counter + abs(len(goal) - len(start))
        if start[index] != goal[index]:
            return counter(num_counter + 1, index - 1)
        return counter(num_counter, index - 1)
    return counter(0, min(len(start), len(goal)) - 1)
    # END PROBLEM 6
...
def minimum_mewtations(start, goal, limit):
    ...
    if limit < 0:  # Fill in the condition
    ...
    elif min(len(goal), len(start)) == 0:  # Feel free to remove or add additional cases
        # BEGIN
        ...
        substitute = minimum_mewtations(start[1:], goal[1:], limit - int(start[0] != goal[0])) + int(start[0] != goal[0])
...
def time_per_word(words, times_per_player):
    ...
    for i in range(len(times_per_player)):   
        for index in range(len(times_per_player[i]) - 1):
            new_list = new_list + ([times_per_player[i][1] - times_per_player[i][0]])
...
def fastest_words(match):
    ...
    # BEGIN PROBLEM 10
    final_list = [[] for num_player in player_indices]
    for word in word_indices:
        faster = 0
        for player in player_indices:
            if time(match, player, word) < time(match, faster, word):
                faster = player
        final_list[faster] += [word_at(match, word)]
    return final_list
